...
def clean_dem_dir(directory):
    ...
    for fic in os.listdir(directory):
        ...
        if re.search(r"(\.tiff|\.hgt|\.dt2|\.tif)$", abs_file):
...
if __name__ == "__main__":
    ...
    try:
        add_egm_to_dem(cropped_egm, args.t, resample_file_name,
                         cropped_file_name)
    except Exception as excpt:
        logger.critical("cannot add EGM: %s", str(excpt))
        clean_and_die(4, str(excpt), args.t)
