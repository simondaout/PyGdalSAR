...
Usage: clean_r4.py --infile=<path> --outfile=<path>  [--mask=<path>] [--threshold=<value>] [--perc=<value>] [--clean=<values>] [--buff=<value>] [--lectfile=<path>] [--scale=<value>] 
...
if iend-ibeg<ncol and jend-jbeg<nlign:
	...
	# print mcrop
	for j in xrange(buf):
	  # print iend1+(buf-j), (np.float(j+1)/buf)
	...
	# print mcrop
	for j in xrange(buf):
	  # print ibeg2-(buf-j), (np.float(j+1)/buf)
	  for i in xrange(jbeg,jend):
