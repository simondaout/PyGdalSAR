...
logger.info('Save time series cube in HDF5 format: disp_cumul_flat.h5')
with h5py.File('disp_cumul_flat.h5', 'w') as f:
    f.create_dataset(
        'maps',
        data=maps_temp,
        dtype='float32',
        compression='gzip',      # Optionnel : pour réduire la taille
        compression_opts=4       # Niveau de compression (0 à 9)
    )
    f.attrs['n_lines'] = new_lines
    f.attrs['n_cols'] = new_cols
    f.attrs['n_dates'] = N
    maps = f['maps'][:] 
...
logger.info('Save elevation map in HDF5 format: elev_map.h5')
with h5py.File('elev_map.h5', 'w') as f:
    f.create_dataset(
        'elev_map',
        data=elev_map_temp,
        dtype='float32',
...
logger.info('Save aspect map in HDF5 format: aspect_map.h5')
with h5py.File('aspect_map.h5', 'w') as f:
    f.create_dataset(
        'aspect_map',
        data=aspect_map_temp,
        dtype='float32',
        compression='gzip',      # Optionnel : pour réduire la taille
        compression_opts=4       # Niveau de compression (0 à 9)
    )
...
logger.info('Save rms map in HDF5 format: rms_map.h5')
with h5py.File('rms_map.h5', 'w') as f:
    f.create_dataset(
        'rms_map',
        data=rms_map_temp,
        dtype='float32',
...
def init_worker(hdf5_paths, shape_2d, shape_3d):
    global _global_data
    _global_data = {}

    # Ouverture des fichiers
    _global_data['maps_file'] = h5py.File(hdf5_paths['maps'], 'r')
    _global_data['models_file'] = h5py.File(hdf5_paths['models'], 'r')
    _global_data['elev_file']   = h5py.File(hdf5_paths['elev'], 'r')
    _global_data['aspect_file'] = h5py.File(hdf5_paths['aspect'], 'r')
    _global_data['rms_file']    = h5py.File(hdf5_paths['rms'], 'r')

    # Lecture des datasets
    _global_data['maps']      = _global_data['maps_file']['maps'][:]
    _global_data['models']      = _global_data['models_file']['models'][:]
    _global_data['elev_map']    = _global_data['elev_file']['elev_map'][:]
    _global_data['aspect_map']  = _global_data['aspect_file']['aspect_map'][:]
    _global_data['rms_map']     = _global_data['rms_file']['rms_map'][:]

    # Enregistre la fermeture automatique
    atexit.register(close_worker_files)


def close_worker_files():
    for key in ['models_file', 'elev_file', 'aspect_file', 'rms_file']:
        try:
            if key in _global_data:
                _global_data[key].close()
        except Exception:
            pass
...
def empirical_cor_wrapper(l, ibeg_emp, iend_emp, mintopo, maxtopo, topofile, perc_los, threshold_rms, threshold_mask, emp_sampling):
    ...
    disp_map = as_strided(maps[:, :, l])
    model_map = as_strided(models[:, :, l])

    if topofile is not None:
        corrected_map, topo_map, residuals = empirical_cor(
        ...
        corrected_map, residuals = empirical_cor(
            l, disp_map, model_map, elev_map, aspect_map, rms_map,
            ibeg_emp, iend_emp, mintopo, maxtopo,
            topofile, perc_los, threshold_rms, threshold_mask, emp_sampling
        )
    return corrected_map, residuals 
...
def temporal_decomp_chunk(chunk, sigma, cond, ineq, eguality):
    # chunk: (N, nb_pixels)
    N, P = chunk.shape
    ...
    for i in range(P):
        disp = chunk[:, i]
        m, sigmam, model = temporal_decomp(disp, sigma, cond, ineq, eguality)
        m_all[i, :] = m
        sigmam_all[i, :] = sigmam
        models_all[i, :] = model
   
    index = np.logical_or(models_all>9999., models_all<-9999)
    models_all[index] = 0. 
    diff_squared = (np.nan_to_num(chunk.T, nan=0) - np.nan_to_num(models_all, nan=0))**2
    res_pixel = np.sqrt(np.nanmean(diff_squared, axis=(0,)))  # shape: (N,)

    return m_all, sigmam_all, models_all, res_pixel

def temporal_decomp(disp, sigma, cond, ineq, eguality):
...
# Create output file to store models
logger.info('Save modeled time series cube in HDF5 format: disp_cumul_models.h5')
with h5py.File('disp_cumul_models.h5', 'w') as f:
    f.create_dataset(
        'models',
        data=np.zeros((new_lines, new_cols, N)),
        dtype='float32',
        compression='gzip',      # Optionnel : pour réduire la taille
        compression_opts=4       # Niveau de compression (0 à 9)
    )
...
for ii in range(int(arguments["--niter"])):
    ...
    if (ii==0) or (arguments["--spatialiter"]=='yes') :
      ...
      with h5py.File('residuals_emp.h5', 'w') as f:
        f.create_dataset(
        'residuals_emp',
        shape=(N,),
        dtype='float32',
      ...
      if arguments["--topofile"] is not None: 
        with h5py.File('maps_topo.h5', 'w') as f:
            f.create_dataset(
            'maps_topo',
            ...
            compression='gzip',
            compression_opts=4,
            data=np.zeros((new_lines, new_cols, N), dtype='float32')
            )

      # tableaux partagés
      hdf5_paths = {
        'maps': 'disp_cumul_flat.h5',
        'models': 'disp_cumul_models.h5',
        'elev': 'elev_map.h5',
        'aspect': 'aspect_map.h5',
        'rms': 'rms_map.h5',
        'maps_topo': 'maps_topo.h5'
          }
      
      nprocess = min(N, nproc)
      for t in range(0, N, nprocess):
        end_t = min(t + nprocess, N)

        with multiprocessing.Pool(processes=nprocess, initializer=init_worker, initargs=(hdf5_paths, (new_lines, new_cols), (new_lines, new_cols, N))) as pool:
            results = pool.starmap(empirical_cor_wrapper, [ (l, ibeg_emp, iend_emp, mintopo, maxtopo, arguments["--topofile"], arguments["--perc_los"], arguments["--threshold_rms"], arguments["--threshold_mask"], arguments["--emp_sampling"]) for l in range(t, end_t) ])
             
        # Écriture des résultats
        if arguments["--topofile"] is not None:
            with h5py.File('disp_cumul_flat.h5', 'r+') as f_data, \
            h5py.File('residuals_emp.h5', 'r+') as f_resid, \
            h5py.File('maps_topo.h5', 'r+') as f_topo:
                for i, (corrected_map, topo_map, residuals) in enumerate(results):
                    l = t + i
                    f_data['maps'][:, :, l] = corrected_map
                    f_topo['maps_topo'][:, :, l] = topo_map
                    f_resid['residuals_emp'][l] = residuals
 
        else:
            with h5py.File('disp_cumul_flat.h5', 'r+') as f_data, \
            h5py.File('residuals_emp.h5', 'r+') as f_resid:
                for i, (corrected_map, residuals) in enumerate(results):
                    l = t + i
                    f_data['maps'][:, :, l] = corrected_map
                    f_resid['residuals_emp'][l] = residuals 
    ...
    if (arguments["--aps"] is None and ii == 0):
        ...
        # Charger les valeurs depuis le fichier HDF5
        with h5py.File('residuals_emp.h5', 'r') as f:
            in_aps = f['residuals_emp'][:]  # Chargement en mémoire (array 1D de taille N)
            ...
            if maxaps > 0:
                ...
                logger.warning("Max APS is zero, skipping normalization.")
                in_aps[:] = 0.5  # Valeur neutre pour éviter une division par zéro

            in_aps[imref] = 0.5  # Valeur centrale pour le pixel de référence

            # Limiter les valeurs basses au 2e percentile
            min_aps = np.nanpercentile(in_aps, 2)
            in_aps[in_aps < min_aps] = min_aps

            # Pondération par le RMS 
    ...
    with h5py.File('residuals_temp.h5', 'w') as f:
        f.create_dataset(
        'residuals_temp',
        shape=(N,),
        dtype='float32',
    ...
    if N < 30:
     with h5py.File('disp_cumul_flat.h5', 'r') as f_data:
        ...
        if arguments["--topofile"] is not None:
          with h5py.File('maps_topo', 'r') as f_topo:
            maps_topo = f_topo['maps_topo'][:, :, :]
            nfigure +=1
    ...
    with TimeIt():
      with multiprocessing.Pool(processes=nproc, initializer=init) as pool:
        for line in range(0, new_lines, block_size):
            ...
            with h5py.File('disp_cumul_flat.h5', 'r') as f_data:
               temp_array = f_data['maps'][line:end_line, :, :][:]
            temp_array = as_strided(temp_array) 
            ...
            m_chunks, sigmam_chunks, models_chunks, res_pixel_chunk = zip(*results)
            ...
            # moyenne des residus entre les chunks
            weights = np.array([chunk.shape[1] for chunk in chunks])  # chaque chunk est (N, nb_pixels_chunk)
            res_pixel_chunks = [r for r in res_pixel_chunk]  # liste de vecteurs (N,)
            res_pixel_stack = np.stack(res_pixel_chunk, axis=0)  # shape: (nproc, N)
            res_pixel = np.average(res_pixel_stack, axis=0, weights=weights)  # shape: (N,)
             
            # stocker les tableaux 
            with h5py.File('disp_cumul_models.h5', 'r+') as f_models:
                f_models['models'][line:end_line, :, :] = models_temp.reshape(block_size_local, new_cols, N)
            with h5py.File('residuals_temp.h5', 'r+') as f_res:
                res_pixel_old = f_res['residuals_temp'][:]
                f_res['residuals_temp'][:] = (res_pixel + res_pixel_old) / 2
            ...
            # nettoyage
            del m, sigmam, m_chunks, sigmam_chunks, models_chunks, models_temp, results, temp_array
            gc.collect() 
    
    # compute RMSE
    # remove outiliers
    with h5py.File('residuals_temp.h5', 'r') as f_res:
      res =  f_res['residuals_temp'][:] 
...
if N < 30:
  ...
  with h5py.File('disp_cumul_flat.h5', 'r') as f_data, \
  h5py.File('disp_cumul_models.h5', 'r') as f_models:
    maps = f_data['maps'][:, :, :] 
    models = f_models['models'][:, :, :] 
