...
def correct_int_era_recons(m_date, s_date, ramp_pars, ramp_pars_recons, maps='yes', phase_era='yes'):
    ...
    return m_date, s_date, ie_gradient, ie_correlation
...
if __name__ == '__main__':
    ...
    ramp_pars[:,0] = [i[0] for i in ramps_results]
    ramp_pars[:,1] = [i[1] for i in ramps_results]
    ramp_pars[:,2] = [i[2] for i in ramps_results]
    ...
    np.savetxt(int_edir+'/phase-era_stats_estim.txt', results_array, fmt='%i, %i, %.3f, %.3f', delimiter=' ')
