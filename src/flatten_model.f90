ccherche le meilleur fit des franges de l interfero avec une fonction : d f/d stack = acycl0 
...
c
c Critere pour selectionner pente phase/stak : ampmax, sigma_stack, coh
...
c
      parameter(nx1=12000,nsumymax=1000,nregx=28,ny1=80000)
      ...
      if (nargs.lt.6) then
       print*,
     $  'Syntax: flatten_stack infile infile_filtered stack_file 
...
c test avec valeur plus cool bc of earthquake: 0.01
c        seuil_cor=0.05
         seuil_cor=0.01
...
c
      open(2,file='ncycle_stack',status='unknown')
      open(5,file='ncycle_stack_az',status='unknown')
...
c comprend pas Dstrack
      Dstack=10
c     nb max de cycles de phase fonction de stack (sur Dstack)
      cyclmax=2
c     nb de pas par cycle: divide cyclmax par ismax parts
      npascycl=30
...
c      print*, 'kx, ky, stackm,islopef,amax,coh,kx,ky,zmin,zmax,sigmaz'
c     boucle en azimuth
      kj=0
      do ky=1,nregy
        iyb=((ky-1)*nsumy)/2+1
...
c         check minmax stack
          if(stack(i,jind).lt.seuil_stack)then 
            phamodf(i,jind)=cmplx(0.,0.)
          endif
...
c           check NaN
c            if(pham(i,jind).gt.10000.and.pham(i,jind).lt.-10000)then 
                ic=ic+1
                weight=weight+amp
                stackm=stackm+stack(i,jind)
                sigmaz=sigmaz+stack(i,jind)**2
c               write(4,*)stack(i,jind),pha
...
c            endif
        ...
        do islope=-ismax,ismax,1
          ...
          ampsump=ampsump/ic
c         maximize the amplitude not the coherence ? 
...
c        close(4)
c
        write(2,'(2f7.0,f8.2,i4,2f8.2,2i4,3f8.2)')
     $    (kx-0.5)*nxr/nregx+0.5,
     $    (ky-0.5)*nyr/nregy+0.5,
     $    stackm,islopef,amax,coh,kx,ky,zmin,zmax,sigmaz
        
c
c         if(coh.gt.seuil_cor.and.sigmaz.gt.seuil_sz)then 
c         coh does not make sense for eq, but need to avoid
c         non-earthquake areas
c        write(*,*)coh,seuil_cor,sigmaz,seuil_sz  

        if(coh.gt.seuil_cor.and.sigmaz.gt.seuil_sz)then 
         write(*,'(2f7.0,f8.2,i4,2f8.2,2i4,3f8.2)')
     $    (kx-0.5)*nxr/nregx+0.5,
     $    (ky-0.5)*nyr/nregy+0.5,
     $    stackm,islopef,amax,coh,kx,ky,zmin,zmax,sigmaz

c          write(*,*)(kx-0.5)*nxr/nregx+0.5, (ky-0.5)*nyr/nregy+0.5, 
c     $         islopef*fac
c           write(*,*)
