...
Usage: plot_los_gps.py --network=<path> --reduction=<path> [--wdir=<path>] [--extension=<value>] 
...
ts_plots = path.join(wdir, "ts_plots/")
makedir(ts_plots)
...
for jj in range((manifold.Npoints)):
    ...
    fig.savefig('ts_plots/Disp_{}.pdf'.format(pt.name.decode('utf-8')), format='PDF')
    del fig
